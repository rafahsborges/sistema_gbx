                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                           href="{{ url('admin/pontos/create') }}" role="button"><i
                                class="fa fa-plus"></i>&nbsp; {{ trans('admin.ponto.actions.create') }}</a>
                    </div>
                        ...
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                ...
                                <tr>
                                    ...
                                    <th is='sortable'
                                        :column="'logradouro'">{{ trans('admin.ponto.columns.logradouro') }}</th>
                                    <th is='sortable' :column="'numero'">{{ trans('admin.ponto.columns.numero') }}</th>
                                    <th is='sortable'
                                        :column="'complemento'">{{ trans('admin.ponto.columns.complemento') }}</th>
                                    <th is='sortable' :column="'bairro'">{{ trans('admin.ponto.columns.bairro') }}</th>
                                    <th is='sortable'
                                        :column="'id_cidade'">{{ trans('admin.ponto.columns.id_cidade') }}</th>
                                    <th is='sortable'
                                        :column="'id_estado'">{{ trans('admin.ponto.columns.id_estado') }}</th>
                                    <th is='sortable' :column="'cep'">{{ trans('admin.ponto.columns.cep') }}</th>
                                    <th is='sortable'
                                        :column="'estacao'">{{ trans('admin.ponto.columns.estacao') }}</th>
                                    <th is='sortable'
                                        :column="'entidade'">{{ trans('admin.ponto.columns.entidade') }}</th>
                                    <th is='sortable'
                                        :column="'latitude'">{{ trans('admin.ponto.columns.latitude') }}</th>
                                    <th is='sortable'
                                        :column="'longitude'">{{ trans('admin.ponto.columns.longitude') }}</th>
                                    <th is='sortable' :column="'altura'">{{ trans('admin.ponto.columns.altura') }}</th>
                                    <th is='sortable'
                                        :column="'id_cliente'">{{ trans('admin.ponto.columns.id_cliente') }}</th>
                                ...
                                <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="17">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a
                                                    ...
                                                    @click="onBulkItemsClickedAll('/admin/pontos')"
                                        ...
                                        <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                        @click="bulkDelete('/admin/pontos/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/pontos/create') }}"
                                   role="button"><i
                                        class="fa fa-plus"></i>&nbsp; {{ trans('admin.ponto.actions.create') }}</a>
                            </div>
                        </div>

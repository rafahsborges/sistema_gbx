                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                           href="{{ url('admin/itens/create') }}" role="button"><i
                                class="fa fa-plus"></i>&nbsp; {{ trans('admin.item.actions.create') }}</a>
                    </div>
                        ...
                        <div class="card-block">
                            <form @submit.prevent="">
                                ...
                                <div class="row" v-if="showAdvancedFilter">
                                    <div class="col col-lg-6 col-xl-6 form-group">
                                        <multiselect v-model="statusesMultiselect"
                                                     :options="{{ $statuses->map(function($status) { return ['key' => $status->id, 'label' =>  $status->nome]; })->toJson() }}"
                                                     label="label"
                                                     track-by="key"
                                                     placeholder="{{ __('Type to search a status/s') }}"
                                                     :limit="2"
                                                     :multiple="true">
                                        </multiselect>
                                    </div>
                                    <div class="col col-lg-6 col-xl-6 form-group">
                                        <multiselect v-model="etapasMultiselect"
                                                     :options="{{ $etapas->map(function($etapa) { return ['key' => $etapa->id, 'label' =>  $etapa->nome]; })->toJson() }}"
                                                     label="label"
                                                     track-by="key"
                                                     placeholder="{{ __('Type to search a etapa/s') }}"
                                                     :limit="2"
                                                     :multiple="true">
                                        </multiselect>
                                    </div>
                            ...
                            <table class="table table-hover table-listing">
                                ...
                                <tr>
                                    ...
                                    <th is='sortable'
                                        :column="'id_etapa'">{{ trans('admin.item.columns.id_etapa') }}</th>
                                    <th is='sortable'
                                        :column="'id_status'">{{ trans('admin.item.columns.id_status') }}</th>
                                ...
                                <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a
                                                    ...
                                                    @click="onBulkItemsClickedAll('/admin/itens')"
                                        ...
                                        <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                        @click="bulkDelete('/admin/itens/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/itens/create') }}"
                                   role="button"><i
                                        class="fa fa-plus"></i>&nbsp; {{ trans('admin.item.actions.create') }}</a>
                            </div>
                        </div>
